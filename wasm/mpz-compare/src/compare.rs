...
use mpz_ot::{
    chou_orlandi::{Receiver as BaseOTReceiver, Sender as BaseOTSender},
    cot::{DerandCOTReceiver, DerandCOTSender},
    kos::{
        Receiver as KosReceiver, ReceiverConfig, Sender as KosSender, SenderConfig,
    },
};
...
type RealCOTSender = DerandCOTSender<KosSender<BaseOTReceiver>>;
type RealCOTReceiver = DerandCOTReceiver<KosReceiver<BaseOTSender>>;
...
pub async fn compare_u32_in_memory(a: u32, b: u32) -> Result<CompareResult, String> {
    ...
    let (cot_send, cot_recv) = real_cot_pair(delta);
...
pub async fn compare_u32_garbler_with_pump_io(
    ...
    let mut rng = StdRng::seed_from_u64(0);
    let delta = Delta::random(&mut rng);
    let cot_send = real_cot_sender(delta);
...
pub async fn compare_u32_evaluator_with_pump_io(
    ...
    let circuit = compare_u32_circuit();
    let cot_recv = real_cot_receiver();
    let mut ctx = Context::new_single_threaded(io);
    let mut ev = Evaluator::new(cot_recv);
...
fn real_cot_pair(delta: Delta) -> (RealCOTSender, RealCOTReceiver) {
    (real_cot_sender(delta), real_cot_receiver())
}

fn real_cot_sender(delta: Delta) -> RealCOTSender {
    let rcot = KosSender::new(
        SenderConfig::default(),
        Block::from(delta.into_inner()),
        BaseOTReceiver::new(),
    );
    DerandCOTSender::new(rcot)
}

fn real_cot_receiver() -> RealCOTReceiver {
    let rcot = KosReceiver::new(ReceiverConfig::default(), BaseOTSender::new());
    DerandCOTReceiver::new(rcot)
}
